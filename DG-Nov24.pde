class Caja{
  ...
  Caja(float x_, float y_){
    text = floor(random(2));
    w = texturas.get(text).width;
    h = texturas.get(text).height;
    c = color(random(255),random(255),random(255));
    makeBody(new Vec2(x_,y_),w,h);
    boolean noEsNegro = true;
    while(noEsNegro){
      int ppix = floor(random(texto.pixels.length));
      if(texto.pixels[ppix] == color(0)){
        pa = new Vec2(ppix%texto.width,ppix/texto.width);
        noEsNegro = false;
      }
    }
  }
  ...
  void makeBody(Vec2 centro, float w_, float h_){
    ...
    ps.setAsBox(AnchoEnBox2D*0.7,AltoEnBox2D*0.7);
...
void setup(){
  ...
  texturas = new ArrayList <PImage>();
  texturas.add(loadImage("2.png"));
  texturas.add(loadImage("gatocubrebocas.png"));
  texto = createGraphics(width,height);
  texto.beginDraw();
  texto.background(255);
  texto.fill(0);
  texto.textSize(600);
  texto.textAlign(CENTER,CENTER);
  texto.text("F",width/2,height/3);
  texto.endDraw();
  for(int i = 0; i<80; i++){
    cajas.add(new Caja(random(width),random(height)));
  }
}
